...
int findCelebrity(int n) {
    ...
    while (stk.size() != 1) {
        ...
        // if person1 knows person2 means person1 cannot be the celebrity, but person2 
        // can be.

        if (knows(person1, person2)) { 
            stk.push(person2);
        }
        // if person2 knows person1 means person2 cannot be the celebrity, but person1 
        // can be.
    ...
    // step 3: check the potential celebrity left in stack knows noone & everybody 
    // knows him.
    ...
    for (int i=0; i<n; i++) {
        if (i != potentialCelebrity) {
            ...
            if (knows(potentialCelebrity, i) == true || knows(i, potentialCelebrity) == false) {
                return -1;
            }
        }
    }
