...
int maxXOR(int n, int m, vector<int> &arr1, vector<int> &arr2) {
    // step 1: insert all elements of arr1 in trie
    ...
    // step 2: Check every element of arr2 by taking xor with all elements in trie

    int maxAns = 0;

    for (int i=0; i<m; i++) {
        int maximumXor = trie.max_xor(arr2[i]);

        maxAns = max(maxAns, maximumXor);
    }
